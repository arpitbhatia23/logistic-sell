...
// Note: client should ideally be imported only where fetching happens,
// and `urlFor` must be adapted for use with `next/image` if possible,
// but for flexibility, I'll keep the direct URL path for <img> tags and use the Image component where possible.
// For Sanity images in Next.js, it's often easiest to use <img> with the full URL
// generated by urlFor, or configure the Next.js image domain for Sanity.
...
/** Represents a Sanity category object. */
interface Category {
  title: string;
}

/** Represents a Sanity author object. */
interface Author {
  name: string;
  image?: {
    asset: {
...
/** Represents a Sanity post object. */
interface Post {
  _id: string;
  title: string;
  slug: {
  ...
  excerpt: string;
  body: any; // Sanity Portable Text body
  _createdAt: string;
  categories: Category[];
  author: Author;
...
const Blog: React.FC = () => {
  ...
  useEffect(() => {
    const query = `*[_type == "post"] | order(publishedAt desc){
      ...
      author->{name, image},
      mainImage
    }`; // Simplified mainImage query - assuming urlFor handles it
        ...
        const processedData = data.map((post) => {
          ...
          if (!finalExcerpt && post.body) {
            ...
            const firstBlock = Array.isArray(post.body)
              ? post.body.find((block: any) => block._type === "block")
              : null;
